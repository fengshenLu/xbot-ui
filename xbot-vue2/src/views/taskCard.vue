        ...
        <div class="flex-row">
          <div class="flex-1 sc-title">
            ...
            <span style="overflow: hidden;
                  ...
                  white-space:nowrap;">
                  {{`${taskInfo.masterProjectName ? taskInfo.masterProjectName + '-'  : ''}${taskInfo.projectName}`}}
            </span>
          </div>
...
<!--          <span v-if="taskInfo.mainUser && taskInfo.mainUser.length >= 4">{{taskInfo.mainUser.map(item => item.name).slice(0,3).join(' ')}}  等{{taskInfo.mainUser.length}}人</span>-->
    ...
    <el-drawer
      ...
      >
      <span slot="title" class="drawer-title">{{`${taskInfo.masterProjectName ? taskInfo.masterProjectName + '-'  : ''}${taskInfo.projectName}`}}</span>
